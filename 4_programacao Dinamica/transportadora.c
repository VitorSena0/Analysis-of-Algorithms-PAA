...
typedef struct {
    float value;   // Valor máximo atingido neste estado
    int prevW;     // Capacidade de peso anterior
    int prevV;     // Capacidade de volume anterior
    int itemIndex; // Índice do item (no vetor de pacotes disponíveis) que foi usado para atualizar este estado; -1 se nenhum
} DPState;
...
*/
Pacote **resolverMochilaParaVeiculo(Veiculo *veiculo, Pacote **pacotesDisponiveis,
                                               int quantidadePacotesDisponiveis, int *quantidadePacotesEscolhidos) {
    ...
    // O índice é calculado como: (i * (dimensaoPeso * dimensaoVolume)) + (p * dimensaoVolume) + (v)
    #define DP(i, p, v) tabelaDP[(i) * tamanhoPorItem + (p) * dimensaoVolume + (v)]
    
    // A camada 0 já está inicializada com 0 (por calloc)
    // Processa cada item (camada de 1 a n)
    for (int i = 1; i <= quantidadePacotesDisponiveis; i++) {
        // Copia toda a camada anterior (i-1) para a camada atual (i)
        memcpy(&DP(i, 0, 0), &DP(i - 1, 0, 0), sizeof(float) * tamanhoPorItem);
        
        // Obtemos as propriedades do item atual em variáveis locais
    ...
    // Percorre as camadas de trás para frente para identificar quais itens foram incluídos
    for (int i = quantidadePacotesDisponiveis; i >= 1; i--) {
        // Se o valor na camada i for diferente do valor na camada i-1, o item i foi adicionado
        if (fabs(DP(i, pesoRestante, volRestante) - DP(i - 1, pesoRestante, volRestante)) > TOLERANCIA) {
            vetorPacotesEscolhidos[contador++] = vetorPacotesAux[i];
            // Atualiza as capacidades restantes subtraindo os recursos ocupados pelo item
            pesoRestante -= vetorPacotesAux[i]->peso;
            volRestante  -= vetorPacotesAux[i]->volume;
        }
    }
    
    // Inverte a ordem dos itens para manter a sequência original de inclusão
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < numeroVeiculos; i++) {
        ...
        vetorVeiculos[i].pesoLimite = pesoLim;
        vetorVeiculos[i].volumeLimite = volLim;
        vetorVeiculos[i].pesoAtual = 0;
        vetorVeiculos[i].volumeAtual = 0;
        vetorVeiculos[i].valorTransportado = 0.0f;
        vetorVeiculos[i].codigos = NULL;
        vetorVeiculos[i].quantidadeCargas = 0;
    }
    ...
    for (int i = 0; i < numeroVeiculos; i++) {
        ...
        if (qtdDisponiveis > 0) {
            Pacote **pacotesEscolhidos = resolverMochilaParaVeiculo(&vetorVeiculos[i], pacotesDisponiveis, qtdDisponiveis, &quantidadePacotesEscolhidos);
    ...
    for (int i = 0; i < numeroVeiculos; i++) {
        int percentualPeso = (vetorVeiculos[i].pesoLimite > 0) ?
                             (int)round((double)vetorVeiculos[i].pesoAtual * 100 / vetorVeiculos[i].pesoLimite) : 0;
        int percentualVolume = (vetorVeiculos[i].volumeLimite > 0) ?
                               (int)round((double)vetorVeiculos[i].volumeAtual * 100 / vetorVeiculos[i].volumeLimite) : 0;
